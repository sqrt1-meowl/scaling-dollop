...
create table public.drill_units (
  ...
  description text not null default '',
  sort_order integer not null,
  worked_example_count integer not null default 3 check (worked_example_count between 2 and 3),
  easy_question_count integer not null default 5 check (easy_question_count between 5 and 10),
  medium_question_count integer not null default 5 check (medium_question_count between 5 and 10),
  hard_question_count integer not null default 3 check (hard_question_count = 3),
...
create table public.drill_unit_progress (
  ...
  drill_unit_id text not null references public.drill_units(id) on delete cascade,
  easy_completed integer not null default 0,
  easy_total integer not null default 5,
  medium_completed integer not null default 0,
  medium_total integer not null default 5,
  hard_completed integer not null default 0,
  hard_total integer not null default 3,
  stage text not null default 'examples' check (stage in ('examples', 'easy', 'medium', 'hard', 'video', 'complete')),
  status public.topic_status not null default 'available' check (status <> 'review'),
  updated_at timestamptz not null default now(),
